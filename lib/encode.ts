...
function decode<P extends bitcoin.Payment, O>(
  p: P,
  f: (payment: P, options?: O) => P,
  t: Script
): {hash: string; format: Script} | undefined {
  try {
    const pay = f(p);
    const hash = pay.hash ? '0x' + pay.hash.toString('hex') : '';
...
export function decodeBtcAddress(
  ...
  | undefined {
  return (
    decode({address, network}, bitcoin.payments.p2sh, Script.p2sh) ||
