...
describe('OptionPairFactory.sol', () => {
  ...
  beforeEach('should deploy option pair factory', async () => {
    ...
    [optionFactory, collateral, referee] = await Promise.all([
      deploy0(alice, OptionPairFactoryFactory),
  ...
  it('should fail to create an option pair with unsupported collateral', async () => {
    ...
    const result = optionFactory.createPair(expiryTime, windowSize, strikePrice, collateral.address, referee.address);
  ...
  it('should fail to create an expired option pair', async () => {
    ...
    const result = optionFactory.createPair(getTimeNow(), windowSize, strikePrice, collateral.address, referee.address);
  ...
  it('should fail to create a zero-window option pair', async () => {
    await collateral.mock.decimals.returns(decimals);
    await optionFactory.enableAsset(collateral.address);
    const result = optionFactory.createPair(expiryTime, 0, strikePrice, collateral.address, referee.address);
  ...
  it('should fail to create an option pair with 0 strikePrice', async () => {
    await collateral.mock.decimals.returns(decimals);
    await optionFactory.enableAsset(collateral.address);
    const result = optionFactory.createPair(expiryTime, windowSize, 0, collateral.address, referee.address);
  ...
  it('should create an option pair with decimals', async () => {
    ...
    const receipt = await optionFactory
      .createPair(expiryTime, windowSize, strikePrice, collateral.address, referee.address)
      .then((tx) => tx.wait(0));
  ...
  it('should generate deterministic contract addresses', async () => {
    ...
    await collateral.mock.decimals.returns(decimals);
    await optionFactory.enableAsset(collateral.address);
    await optionFactory.createPair(expiryTime, windowSize, strikePrice, collateral.address, referee.address);
