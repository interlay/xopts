...
1. Alice creates a timelocked UTXO, locking 10 ``BTC`` for period :math:`t`, e.g., 24h, by broadcasting (and hence including) a transaction :math:`T_A` on Bitcoin
   ...
   * :math:`T_A` has at least one output, which locks 1 ``BTC`` under the following condition: :math:`\sigma_A \land t_{now} > t`, where :math:`\sigma_A` is the signature generated by :math:`sk_A`.  
...
3. Alice submits an SPV proof for :math:`T_A` to the :ref:`xflash-sc` on Ethereum by calling the  function.


  a) The :ref:`xflash-sc` calls :ref:`btc-relay` to (i) verify the inclusion proof for :math:`T_A` and (ii) to parse the transaction and extract the timelock duration from the first UTXO (and the target  `account` specified in the second `OP_RETURN` output).

  b) Mints 10 ``flashBTC`` token to Alice's Ethereum account `in the XFlash contract` by calling the :ref:`mint` function in the :ref:`treasury`.
...
1. Dave requests a *flash loan* by calling ... TODO ... (@Dominik)

2. ... Flash loan is executed.

3. Dave returns :math:`L` ``flashBTC`` to the smart contract and pays a fee :math:`f` in ``ETH``.
...
4. The smart contract distributed :math:`f` among Alice, Bob and Carol proportional to their contributions of ``flashBTC``.
...
1. The current time math:`t_{now}` is within a *security delay* :math:`\delta` of  expiration time :math:`t` 

  * This is necessity since the block generation rates in Bitcoin and Ethereum can "desychronize", that is Bitcoin can speed up or slow down in relation to Ethereum due to mining variance. We can only avoid this if we have cross-chain relative timelocks (WIP).

2. The smart contract stops accepting any new flash loan requests and liquidates Alice's balance. 
...
Post-conditions
...............
There are only 90 ``flashBTC`` left in the smart contract, and Alice has regained full control over her ``BTC``. 


Extend 
-------

Alice extends the timelock of her locked 10 BTC by renewing the timelock of `TX_A` **before** :math:`t - \delta`. 
