...
contract Obligation is IObligation, IERC20, European, Ownable, WriterRegistry {
    ...
    event RequestExercise(address indexed buyer, address indexed seller, bytes32 id, uint amount, uint secret);

    /// @notice Emit upon successful exercise execution (tx inclusion & verification).
    event ExecuteExercise(address indexed buyer, address indexed seller, uint amount);
    ...
    **/
    function getSecret(bytes32 id) external view returns (uint) {
    ...
    **/
    function executeExercise(
        ...
        uint satoshis = IReferee(referee)
            .verifyTx(
                height,
                index,
                txid,
    ...
    function _verifyReceipt(address buyer, bytes32 id, uint satoshis) internal {
        uint amount = _requests[buyer][id].amount;
